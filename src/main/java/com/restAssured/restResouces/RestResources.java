...
public class RestResources {
    ...
    public RequestSpecification setUpRequest(String baseUri, Map<String, Object> headers, Map<String, Object> queryParams, Map<String, Object> pathParams, ContentType contentType) {
        RequestSpecification request = RestAssured.given().log().all().baseUri(baseUri).contentType(contentType).accept(contentType);
        if (headers != null) {
    ...
    public Response get(RequestSpecification request, String endpoint) {
        return request.log().all().when().get(endpoint);
    }
    ...
    public void doVerifySchema(Response response, String schemaFilePath) {
        try {
            response.then().log().all().assertThat().body(JsonSchemaValidator.matchesJsonSchemaInClasspath(schemaFilePath));
    ...
    public <T> T doVerifyValueInResponse(Response response, String path, Class<T> type) {
    ...
    public void doVerifyResponseTime(Response response) {
        response.then().log().all().time(lessThanOrEqualTo(200L), TimeUnit.MICROSECONDS);
    }
}